...
struct CoordinateDials: View {
    ...
    var body: some View {
        HStack(spacing: 24) {
            DialView(label: Strings.BodyDetail.rightAscension, value: ra.hmsString,  kind: .ra(ra),   accent: accent)
            DialView(label: Strings.BodyDetail.declination,    value: dec.dmsString, kind: .dec(dec), accent: accent)
        }
    }
}
...
struct DialView: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            ...
            DialCanvas(kind: kind, accent: accent)
