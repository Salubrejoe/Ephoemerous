...
enum EProjection {
    typealias PointSet = [CGPoint?]
    typealias Vector🏹 = SIMD3<Double>
    
    
    static let obliquity: Angle = .degrees(23.4393)
    
    static func project(_ Q      : Vector🏹,
                        origin  O: Vector🏹,
                        plane   P: Vector🏹) -> CGPoint? {
    ...
    static func sampleEcliptic(
        ...
        (0...steps).map { i in
            ...
            let λ = t * 2 * .pi
            let β = 0.0
            let θ = appState.precessedSiderealOffset.radians
