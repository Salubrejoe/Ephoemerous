...
struct ENSMilkyWayLayer: EGridLayer {
...
//        dc.ctx.fill(bg, with: .color(Color(hue: 0.64, saturation: 0.85, brightness: 0.06)))
        ...
        for b in latStrips {
            ...
            let pts = EProjection.sampleCurve(appState: dc.state, mode: .northSouth) { t in
        ...
        let gcVec = EGalacticCoords.equatorialVector(l: 0, b: 0)
            .sidereallyRotated(by: offset)
        if let gcProj = EProjection.project(gcVec, appState: dc.state, mode: .northSouth) {
