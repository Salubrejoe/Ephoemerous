...
struct ENSEquatorTropicsLayer: EGridLayer {
    ...
    private func drawEquatorAndTropics(in dc: inout EGraphicContext) {
        
        let equator  = EProjection.sampleCurve(
            appState: dc.state,
            mode: .northSouth
        ) { step in
