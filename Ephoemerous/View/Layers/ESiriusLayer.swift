...
struct ESiriusLayer: EGridLayer {
    ...
    private static let trueRA:  Double = catalogRA_h  * 15 * .pi / 180
    /// True J2000 Dec in radians
    private static let trueDec: Double = catalogDec_d * .pi / 180
    ...
    func draw(in dc: inout EGraphicContext) {
        ...
        if abs(date.timeIntervalSince(Self.lastLoggedDate)) > 0.5 {
            ...
            logPipeline(date: date, modelStar: modelStar, siderealOffset: dc.state.siderealOffset)
        }
        ...
        let (c, s) = (cos(θ), sin(θ))
        let v = ECalAndTransManager.equatorialVector(ra: pRA, dec: pDec)
    ...
    private func logPipeline(date: Date, modelStar: EStar?, siderealOffset: Double) {
        ...
        print("    RA  \(raString(Self.trueRA))  (\(degStr(Self.trueRA * 180 / .pi))°)")
        print("    Dec \(decString(Self.trueDec))  (\(degStr(Self.trueDec * 180 / .pi))°)")
        ...
        print("    RA  \(raString(pRA))")
        print("    Dec \(decString(pDec))")

        // What the model actually stores
        if let star = modelStar {
            let mRA  = star.rightAscension.radians
            let mDec = star.declination.radians
            print("  [EStar model (brightest star by magnitude)]")
            print("    name \(star.name)  magnitude \(star.magnitude)")
            print("    stored RA  \(raString(mRA))  (\(degStr(mRA * 180 / .pi))°)")
            print("    stored Dec \(decString(mDec))  (\(degStr(mDec * 180 / .pi))°)")

            let (mpRA, mpDec) = ECalAndTransManager.precess(ra: mRA, dec: mDec, to: date)
            print("  [Precessed to date — model]")
            print("    RA  \(raString(mpRA))")
            print("    Dec \(decString(mpDec))")
        }
