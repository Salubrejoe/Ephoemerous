...
enum Projection {
    ...
    /// properties on `AppState`; resolving them once per frame (see
    /// `GraphicContext`) and threading this value keeps the per-point
...
//        if simd_dot(e2, O) < 0 { e2 = -e2 }
    ...
    static func sampleEcliptic(
        ...
        (0...steps).map { i in
            ...
            let Q = Precession.eclipticVector(
                atStep: t,
                siderealOffset: siderealOffset
            )
            return Projection.project(Q, viewpoint: viewpoint)
        }
    }
}
