...
struct GraphicContext {
    ...
    init(ctx:                     GraphicsContext,
         ...
         viewpoint:               Projection.Viewpoint,
    ...
    // `Artist.drawPOILabel` for what `promotion` / `wiggle` drive.
    ...
    /// `SkyObject.id`), as a function of the per-frame animation clock:
...
//    }
    ...
    /// `viewpoint.skyPoint` → `Projection.project` → `toScreen` — so the
    ...
    func screenPoint(azimuth: Double, altitude: Double) -> CGPoint? {
        ...
        guard let p = Projection.project(v, viewpoint: viewpoint) else { return nil }
        return toScreen(p)
    }
    ...
    //
    // Asset-catalog colours (`Color("grid")`, every `Palette` entry) are
