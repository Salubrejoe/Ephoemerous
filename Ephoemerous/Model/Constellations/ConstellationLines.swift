...
// resolves every segment into a concrete `(Star, Star)` pair against
...
// `Constellation.rawValue`) and each value is an array of two-element
...
final class ConstellationLines {
    ...
    private init() {
        ...
        for (key, pairs) in raw {
            guard let cons = Constellation(rawValue: key) else { continue }
            var segs: [Segment] = []
    ...
    private static func loadRaw() -> [String: [[String]]] {
        guard let url = Bundle.main.url(forResource: "constellationLines", withExtension: "json") else {
            Logger.constellationLines("constellationLines.json not found in bundle")
            return [:]
        }
        do {
            ...
            Logger.constellationLines("decode failed: \(error)")
            return [:]
        }
    }
    ...
    private typealias Index = [Constellation: [String: Star]]

    private static func buildIndex(stars: [Star]) -> Index {
        var idx: Index = [:]
        for star in stars {
            let cons = star.constellation
            guard cons != .none else { continue }
            // `Star.name` is e.g. "21 α And", "α And", or "21 And".
    ...
    private static func resolve(designation: String, in cons: Constellation, using index: Index) -> Star? {
    ...
    private static func centroid(of stars: [Star]) -> (ra: Angle, dec: Angle) {
