...
struct Star: Identifiable, Hashable {
    ...
    /// its heart, once plain white. `SkyObject.id` (`star_<id>`) inherited
    ...
    let spectralClass  : HRClass
    let pmRA           : Double
    let pmDE           : Double
    /// Companion data, when the catalogue has any. `nil` for the majority.
    /// See `StarMultiplicity` for why "is a binary" is NOT the useful
    /// question and `isShowpiece` is.
    let multiplicity   : StarMultiplicity?
    ...
    /// this with zero trig (see `StarsLayer` / `GraphicContext.starCull`).
    ...
    init(from starData: StarData) {
        let ra  = Star.calculateRA(from: starData)
        let dec = Star.calculateDec(from: starData)
        self.name           = Star.calculateName(from: starData)
        self.rightAscension = ra
        self.declination    = dec
        self.magnitude      = Star.calculateMagnitude(from: starData)
        self.spectralClass  = Star.calculateSpectralClass(from: starData)
        self.pmRA           = Star.calculateProperMotionRA(from: starData)
        self.pmDE           = Star.calculateProperMotionDec(from: starData)
        self.multiplicity   = StarMultiplicity(from: starData)
        self.equatorialVector = Precession.equatorialVector(ra: ra, dec: dec)
    }
    
    var constellation: Constellation {
        let components = name.components(separatedBy: " ")
        if let abbrev = components.last, let constellation = Constellation(rawValue: abbrev) {
...
private extension Star {
    ...
    static func calculateName(from starData: StarData) -> String {
        ...
        guard let constellation = Constellation(rawValue: last3) else { return "Unknown" }
...
static func calculateRA(from starData: StarData) -> Angle {
    ...
    static func calculateSpectralClass(from starData: StarData) -> HRClass {
        HRClass(rawValue: String(starData.spectralClass.prefix(1))) ?? .G
    }
...
extension Star {
    static let mockStars: [Star] = [
        Star(from: StarData(name: "Alp Ori", rightAscensionHours: "5", rightAscensionMinutes: "55", rightAscensionSeconds: "10", declinationSign: "-", declinationDegrees: "7", declinationMinutes: "24", declinationSeconds: "25", magnitude: "0.42", spectralClass: "M", pmRA: "-3.2", pmDE: "2.3")),
        Star(from: StarData(name: "Bet CMa", rightAscensionHours: "6", rightAscensionMinutes: "45", rightAscensionSeconds: "9", declinationSign: "-", declinationDegrees: "16", declinationMinutes: "42", declinationSeconds: "58", magnitude: "-1.46", spectralClass: "A", pmRA: "-7.3", pmDE: "3.6")),
        Star(from: StarData(name: "Gam Leo", rightAscensionHours: "10", rightAscensionMinutes: "19", rightAscensionSeconds: "59", declinationSign: "+", declinationDegrees: "19", declinationMinutes: "50", declinationSeconds: "29", magnitude: "2.61", spectralClass: "K", pmRA: "-1.5", pmDE: "0.7"))
    ]
}
