...
enum HRClass: String, CaseIterable {

    case O, B, A, F, G, K, M, unknown

    // MARK: - Colours
    //
    // Both palettes live in `Palette` so every colour in the app
    ...
    /// colour in either appearance. Derived in `Palette` from the one
    /// adaptive asset colour (no more dark/light-as-gradient split).
    var badgeGradient: (top: Color, bottom: Color) {
        Artist.shared.palette.spectralGradient(self)
    }
}
