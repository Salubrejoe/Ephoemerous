...
// MARK: - AppState + Detail
// Canvas-tap → detail-sheet behaviour. One destination, one pan, one
// dismiss. Replaces the old AppState+Sheets.swift (four `present*Info`
...
extension AppState {
    ...
    //  cycle taxonomy. Constellation stories live in ConstellationDetailView.)
    ...
    private func screenPosition(of obj: SkyObject) -> CGPoint? {
        switch obj {
        case .sun:
        ...
        case .star(let star):
            return favouritePositions[SkyObject.star(star).id] ?? screenPosition(of: star)
