...
//   AppState+Time.swift   — observation date, animation clock
//   AppState+Space.swift  — observer geometry, origin/plane
//   AppState+Stars.swift  — star filtering, caching, selection, recents
//   AppState+Sheets.swift — sheet / modal presentation helpers
// Preset & tracking logic lives in FocusPreset.swift.
...
class AppState {
    ...
    // FavouritesLayer. Keyed by `SkyObject.id` so it generalises
    ...
    var constellationLabelHitRects:  [Constellation: CGRect]    = [:]
    ...
    // overlay resolves back to the Star via NamedStarsLayer.star(named:).
    ...
    // `GraphicContext.toScreen`, so each projected point's *position*
    ...
    /// by `MotionService.aim`; toggled via `toggleCompassMode()`.
    ...
    /// In-flight NorthIN↔NorthOUT morph, advanced once per canvas-clock frame
    /// (see `advanceCanvasClock`) so the projection reprojects each tick — the
    /// grid morphs with the labels instead of snapping. `nil` at rest; the
    ...
    // MARK: - Spatial state  (logic → AppState+Space.swift)
    var origin:   Origin
    var plane:    Plane
    ...
    // `refreshLocalityName()` (see AppState+Locality.swift).
    ...
    var _originTransition:     OriginTransition?     = nil   // defined in AppState+Location.swift
    ...
    // as the origin. Not persisted. See AppState+Location.swift.
    var _didAdoptDeviceLocation: Bool = false

    // MARK: - Star state  (logic → AppState+Stars.swift)
    ...
    /// `AppState.magnitudeCap(forScale:)`), so this no longer gates the
    ...
    var magnitudeFilter: Double = AstroConstants.defaultMagCap {
        didSet {
            CloudSync.shared.saveMagnitudeFilter(magnitudeFilter)
        }
    }

    // MARK: - Favourites  (logic → AppState+Favourites.swift)
    ...
    var favourites: [SkyObject] = [] {
        didSet {
            let ids = favourites.map(\.id).joined(separator: ", ")
            Logger.favourites("favourites changed (\(favourites.count)) → [\(ids)]")
            CloudSync.shared.saveFavourites(favourites)
        }
    }
    ...
    @ObservationIgnored var _consSegProj:     [Constellation: [(a: CGPoint, b: CGPoint)?]] = [:]

    // MARK: - Detail destination  (logic → AppState+Detail.swift)
    ...
    var detailDestination: SkyObject? = nil {
        didSet {
    ...
    //  constellation shows its own story in ConstellationDetailView.)
    ...
    // MARK: - Preset backing store  (logic → FocusPreset.swift)
    var _activeTransition: PresetTransition? = nil

    // MARK: - Init
    // At launch CoreLocation has no fix yet, so origin/plane start at their
    // defaults; the first real fix is adopted once via
    // adoptInitialDeviceLocation(_:) (see AppState+Location.swift).
    init() {
        self.origin   = .init()
        self.plane    = .init()
    }
